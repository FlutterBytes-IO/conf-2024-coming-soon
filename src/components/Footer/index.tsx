...
export default function Footer() {
    ...
    return (
    <div className={`pt-[99px] transition-all duration-500 pb-[80px] px-[101px] ${isDarkmode ? "footer-gradient" : ""}`}>
        <div 
        className="border border-[#1167B1] rounded-xl rounded-b-none border-b-0 p-7 pb-5 flex flex-col md:flex-row md:items-center justify-between">
            <div className="flex flex-col gap-y-5 flex-1">
                <Image src={isDarkmode ? FooterLogoDark : FooterLogoLight} alt="logo" className="transition-all duration-500" />
            ...
            <div className="flex flex-col gap-y-5 flex-1">
                <p className={`text-sm transition-all duration-500 ${isDarkmode ? "text-[#E0E1E2]" : "text-[#576672]"}`}>To get the latest updates about FlutterBytes Conference 2024 directly in your email inbox, subscribe below.</p>
                <GetNotifiedForm />
            </div>
        </div>
    </div>)
