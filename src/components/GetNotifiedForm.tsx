...
export default function GetNotifiedForm({
	...
	return (
		<form
			...
			className={`flex bg-transparent transition-all duration-500 items-center rounded-[30px] !border ${
				isDarkmode
					? "!border-[#1167B1]"
					: "!border-[#D0EFFF] md:hover:!border md:hover:!border-[#1167B1] !bg-[#E5F6FF]"
			}`}
		>
			<input
				value={value}
			...
			<Button
				disabled={isLoading}
				type="submit"
				otherstyles={`py-4 min-[321px]:px-5 flex items-center justify-center text-center gap-x-1`}
			>
				{isSubmitted ? (
					<>
						Email Added <TickIcon />
					</>
				) : (
					"Get Notified"
				)}
