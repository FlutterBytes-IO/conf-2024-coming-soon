...
export default function FormModal({ openModal, closeModal }: { openModal: boolean; closeModal: () => void; }) {
    return <div onClick={(e) => e.target === e.currentTarget && closeModal()} className={`fixed w-full transition-all duration-500 z-50 h-full top-0 left-0 ${openModal ? "flex bg-opacity-50" : "hidden bg-opacity-0"} items-center justify-center bg-black/50`} style={{backdropFilter: "blur(5px)"}}>
